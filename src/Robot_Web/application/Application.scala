...
object Application extends App {
  ...
  val urlSearch = "https://www.linternaute.com/restaurant/guide/ville-rennes-35000/?name=" + SyntaxAnalyser.UrlBuilder(SyntaxAnalyser.ExpressionResponse(requete)) 
 
 
  println(urlSearch)
 
  val couplesUrls: List[(String, String)] = AnalysePageImp.resultats(urlSearch, SyntaxAnalyser.ExpressionResponse(requete)) // prend 2.5 secondes à s'éxecuter
  val temps_debut = System.currentTimeMillis()
  val triplets:List[(String,String)] = AnalysePageImp.reponse(urlSearch,SyntaxAnalyser.ExpressionResponse(requete))
  val temps_analyse = System.currentTimeMillis()
  println("temps analyse triplets = " + (temps_analyse - temps_debut) + "ms")
  println("Les couples (titre,url): " + couplesUrls)
  println("Les couples (titre,adresse): " + triplets)
